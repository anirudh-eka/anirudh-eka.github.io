...
Transit = {
	__makeIndividualScenes: function(scenes, els){
		for (var i = 0; i < els.length; i++) {
			...
			scene.addFirstFrame() = function(frameName){
				this.className += frameName;
			}
			...
			if(scene.dataset.fxOther != undefined) {
				...
				for (var key in externalFx) {
				    if (externalFx.hasOwnProperty(key)) {
				    	...
				    	// Todo: add fx/class (which is the value of externalFx) to all of the scene.externalScenesToAffect
