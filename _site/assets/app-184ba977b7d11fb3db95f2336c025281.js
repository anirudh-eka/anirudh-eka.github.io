...
var playMovie = function(index) {
	...
	var externalFx = scene.dataset.fxOther
	for (var key in externalFx) {
	    if (scene.dataset.fxOther.hasOwnProperty(key)) {
	        Transit.getScenes(scenthis[key]);
	    }
	}
...
Transit = {
	getScenes: function(name) {
		var els;
		if(name.startWith(".")){ els = document.getElementsByClassName(name) }
		if(name.startWith("")){ els = document.getElementsByTagName(name) }
		var scenes = {}
		scenes.length = 0;
